#!/usr/bin/env bash
#===============================================================================
# UUFW Firewall Manager v3.0.2
# Change log:
#   - [v3.0.2] 补全 DHCPv6 (UDP 546) 规则，确保 IPv6 环境下的网络稳定性
#   - [v3.0.2] 增加 ca-certificates 和 util-linux (flock) 依赖安装
#   - [v3.0.2] 优化 nftables 持久化：自动管理 /etc/nftables.conf 中的 include 语句
#   - [v3.0.2] 实现 nftables 原子性更新：将 delete table 写入脚本模板，消除空窗期
#   - [v3.0.2] 状态查询增强：新增持久化状态检查逻辑
#   - [v3.0.1] 修复 install_deps() 全局变量刷新、temp_open_port 路径问题
...
load_config() {
    ...
    [[ -n "$LOG_RATE" ]] || LOG_RATE="$DEFAULT_LOG_RATE"
}
...
build_iptables_restore_files() {
...

    {
        ...
        if [[ "${CF_ONLY:-0}" == "1" ]]; then
            emit_iptables_cf_rules_v4 "$cf4_file" "$cf_port_list"
        fi
...

    {
        ...
        if [[ "${CF_ONLY:-0}" == "1" ]]; then
            emit_iptables_cf_rules_v6 "$cf6_file" "$cf_port_list"
        fi
...
apply_nftables() {
    ...
    # [v3.0.2] 持久化增强：自动包含 include 语句
    if [[ -d /etc/nftables ]]; then
        cp -f "$nft_conf" /etc/nftables/uufw.nft
        if [[ -f /etc/nftables.conf ]]; then
            if ! grep -q "include \"/etc/nftables/uufw.nft\"" /etc/nftables.conf; then
                echo "include \"/etc/nftables/uufw.nft\"" >> /etc/nftables.conf
            fi
        fi
...
apply_current() {
    case "$FW_TYPE" in
        ...
        ufw)
            warn "检测到 ufw 正在运行，请先在安装步骤中禁用 ufw 后再应用本脚本规则"
            return 1
            ;;
        *)
            err "未检测到可用防火墙组件"
...
install_deps() {
    ...
    if ! has_cmd nft && ! has_cmd iptables; then
        ...
        case "$OS_TYPE" in
            alpine)
                # [v3.0.2] 增加 ca-certificates 和 util-linux
    ...
    case "$FW_TYPE" in
        nftables)
            ...
            if has_cmd systemd-run; then
                ...
                ("$self" apply >/dev/null 2>&1) >/dev/null 2>&1 &
            fi
            ;;
        iptables)
